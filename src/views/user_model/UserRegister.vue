<template>
  <div id="login-container">
    <h1 style="text-align: center;margin-top: 100px;" >国脉山河用户注册</h1>
    ...
    <div class="container" >
      ...
      <form class="form">
      <div class="input-row">
        <input type="text" id="username" class="input" v-model="user_info.name" required>
        <span>用户名</span>
      </div>
      <div class="input-row">
        <input type="password" id="password" class="input" v-model="user_info.password" required>
        <span>密码</span>
      </div>
      <div class="input-row">
      ...
      <div class="input-row">
        <input type="email" id="email" class="input" v-model="user_info.email" required>
        <span>邮箱</span>
      </div>
      <div class="input-row">
        <input type="tel" id="phone" class="input" v-model="user_info.phone" required>
        <span>手机号</span>
      </div>
      <div class="input-row">
        <label for="role" class="name">用户类型:</label>
        <select id="role" class="input" v-model="user_info.role" required>
          <option value="1">审核员</option>
          <option value="2">标注员</option>
        </select>
      </div>
    ...
    <div class="box" >
      ...
      <form class="form">
          <div class="input-row">
            ...
            <input type="text" id="realName" class="input" v-model="user_info.true_name" required>
            <span>真实姓名</span>
          </div>
          <div class="input-row">
            <label for="gender" class="name">性别:</label>
            <select id="gender" class="select" v-model="user_info.gender" required>
          ...
          <div class="input-row">
            ...
            <input type="text" id="idCard" class="input" v-model="user_info.id_card" required>
            <span>身份证号</span>
          </div>
          <div class="input-row">
            <label for="birthDate" class="name">出生日期:</label>
            <input type="date" id="birthDate" class="select" v-model="user_info.birth_date" required>
            
          </div>
          <div class="input-row">
            <label for="province" class="name">省份:</label>
            <select id="province" class="select" style="" v-model="selectedProvince" @change="updateCities">
            ...
            <select id="district" class="select" v-model="user_info.address">
                <option v-for="district in districts" :value="district">{{ district }}</option>
            </select>
        </div>
        <div class="input-row">
          <label for="introduction" class="name">简介:</label>
          <textarea id="introduction"  style="background-color: #333;width: 100%;padding: 10px;color: #fff;" v-model="user_info.introduction"></textarea>
        </div>
      </form>
...
export default {
  data() {
    return {
      ...
      // selectedProvince: '', // 用于存储选中的省份
      // selectedCity: '',     // 用于存储选中的城市
      cities: [],           // 用于存储城市列表
      user_info: {
        address: '',         // 用于存储选中的区域
      },
      districts: [],         // 区域列表，您可能需要根据城市选择更新此列表
  ...
  computed: {
    isRegistrationValid() {
      ...
      return (
        ...
        this.repeatPassword === this.user_info.password &&
        this.user_info.email &&
        this.user_info.phone &&
        this.user_info.true_name &&
        this.user_info.gender &&
        this.user_info.id_card &&
        this.user_info.birth_date &&
        //this.user_info.address &&
        this.user_info.role // 确保用户类型也选择了
      );
    }
  },
  methods: {
    ...
    const registrationData = {
      ...
      birth_date: this.user_info.birth_date,
      ...
      role: this.user_info.role,
        ...
        // 发送注册请求，包括邀请码
        axios
        .post('http://10.112.244.193:8002/admincenter/register/userinfo', registrationData)
        .then(response => {
          if (response.status === 200) {
            alert('注册成功！');
            console.log('注册成功');
            this.$router.push('/Log/user'); // 注册成功后跳转到登录页面
    ...
    async updateCities() {
      try {
        const response = await axios.get('http://10.112.244.193:8002/search_sub', {
          params: {
