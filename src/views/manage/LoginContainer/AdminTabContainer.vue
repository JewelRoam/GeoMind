<template>
    ...
    <div class="tab-container">
      <button @click="switchToUserType('标注员/审核员')" class="tab tab--1 user-type-button">标注员/审核员</button>
      <button @click="switchToUserType('管理员')" class="tab tab--2 user-type-button admin-button">管理员</button>
  ...
  methods: {
    async login() {
      ...
      if (/\d{11}/.test(this.accountOrPhone)) {
        admin_info.phone = this.accountOrPhone.toString();
      }

      try {
        const response = await axios.post('http://10.112.244.193:8002/log/Admin', admin_info, {
          headers: {
        ...
        if (data.status === 200) {
          ...
          sessionStorage.setItem('user_info.role', '0');
          alert('登录成功！');
          window.close
