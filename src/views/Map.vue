<template>
  <!-- //地图组件的模板，主要包括地图显示区域、
  //搜索框、地图类型选择框、经纬度输入框、返回中心点按钮、放大缩小按钮和叠加物显示等部分。 -->
  <div class="map" ref="map">
    <div class="mapBox" ref="mapBox" @mousedown="onMousedown"></div>

    <!-- 经纬度框 -->
    <div class="posInput">
      <input type="text" placeholder="经纬度..." v-model="posValue" @keyup.enter="onChangePos" @click.stop />
    </div>
    <!-- 返回到初始中心点按钮 -->
    <div class="backCenterBtn" @click.stop="backCenter"></div>
    <!-- 放大缩小按钮 -->
    <div class="scaleBtnBox">
      <div class="scaleBtn in" :class="{ disabled: zoom >= maxZoom }" @click.stop="zoomIn"></div>
      <div class="scaleBtn out" :class="{ disabled: zoom <= minZoom }" @click.stop="zoomOut"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      currentTileCache: {},
      // 初始中心经纬度
      initCenter: [98.524630, 25.010419], // 
      center: [98.524630, 25.010419], // 
      // 初始缩放层级
      zoom: 16,
      // 缩放层级范围
      minZoom: 3,
      ...
      mapList: [

        {
          name: '国脉山河',
          value: 'GMSHImage',
          // urls:['http://10.112.244.193:8080/#background=custom:http://10.112.244.193:8000/map/{z}/{x}/{y}']
          urls: ['http://10.112.244.193:12345/map/{z}/{x}/{y}']
          // urls:['http://10.112.244.193:12345/map/{z}/{x}/{y}']
  ...
  watch: {
    ...
    press_point(newValue) {
      this.$emit('variable-updated', newValue);
    },
    center(newValue) {
      this.$emit('center', newValue)
    }
    //用于监听数据的变化并执行相应的操作。在这段代码中，
    //它监听了center属性的变化，并在val发生变化时将其值转换为以逗号分隔的字符串，并将结果赋给posValue。
  },
  created() {
  ...
  methods: {
    ...
    onMousedown(e) {
      if (e.which === 1) {
        ...
        // let ox = 0 - 512;
        // let oy = 0 - 512;
        let ox = e.offsetX - 512;
        let oy = e.offsetY - 512;
        let [x, y] = (this.selectMapData.lngLatToMercator || lngLatToMercator)(...this.center);
        // 更新拖动后的中心点经纬度


        let oxoff = ox * (this.selectMapData.resolutions || resolutions)[this.zoom];
        let oyoff = oy * (this.selectMapData.resolutions || resolutions)[this.zoom];
        this.press_point = (this.selectMapData.mercatorToLngLat || mercatorToLngLat)(x + oxoff, y + oyoff);

        // let [ax, ay] = (this.selectMapData.lngLatToMercator || lngLatToMercator)(this.press_point[0], this.press_point[1]);
        // let bx = ax - x;
        // let by = ay - y;
        // console.log('[bx,by]', bx, by)
        // let bxoff = bx / (this.selectMapData.resolutions || resolutions)[this.zoom];
        // let byoff = by / (this.selectMapData.resolutions || resolutions)[this.zoom];
        // console.log('[bx,by]', bxoff, byoff)
    ...
    // 返回画线点的坐标
    return_point(){
      return this.press_point;
    },

    draw(list) {
      let [x, y] = (this.selectMapData.lngLatToMercator || lngLatToMercator)(...this.center);
      let [ax, ay] = (this.selectMapData.lngLatToMercator || lngLatToMercator)(list[0], list[1]);
      let bx = ax - x;
      let by = ay - y;
      let bxoff = bx / (this.selectMapData.resolutions || resolutions)[this.zoom] + 512;
      let byoff = by / (this.selectMapData.resolutions || resolutions)[this.zoom] + 512;
      bxoff = Math.round(bxoff);
      byoff = Math.round(byoff);
      // console.log('[bx,by]', bxoff, byoff)
      return [bxoff, byoff]
